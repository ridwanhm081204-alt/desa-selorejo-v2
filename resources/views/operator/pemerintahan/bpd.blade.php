...
<div class="dash-card bg-white p-4 mb-4 border-top border-4 border-success shadow-sm rounded-4 text-start">
    ...
    <form action="{{ url('operator/bpd/hero') }}" method="POST">
        @csrf
        <div class="row g-3">
            <div class="col-md-4">
                <label class="small fw-bold text-muted">Judul Halaman</label>
                <input type="text" name="title" class="form-control form-control-sm" value="{{ $hero['title'] ?? 'Badan Permusyawaratan Desa' }}">
            </div>
            <div class="col-md-5">
                <label class="small fw-bold text-muted">Sub-Judul</label>
                <input type="text" name="subtitle" class="form-control form-control-sm" value="{{ $hero['subtitle'] ?? 'Lembaga legislatif desa sebagai mitra Pemerintah Desa Selorejo.' }}">
            </div>
            <div class="col-md-2">
                <label class="small fw-bold text-muted">Ikon (Lucide)</label>
                <input type="text" name="icon" class="form-control form-control-sm" value="{{ $hero['icon'] ?? 'users-2' }}">
            </div>
...
<div class="card border-0 shadow-sm rounded-4 mb-4 overflow-hidden text-start">
    <div class="card-body p-4 d-flex justify-content-between align-items-center">
        <div>
            <h5 class="fw-bold mb-0">Anggota BPD</h5>
            <small class="text-muted">Kelola jajaran perwakilan warga BPD</small>
        </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                <thead class="bg-light text-uppercase">
                    <tr>
                        <th class="ps-4 py-3 small fw-bold text-muted" style="width: 30%">Nama Lengkap</th>
                        <th class="py-3 small fw-bold text-muted">Jabatan</th>
                        <th class="py-3 small fw-bold text-muted text-center" style="width: 15%">Foto</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="py-3 text-center">
                            ...
                            @else
                                <div class="bg-light rounded-circle d-flex align-items-center justify-content-center mx-auto shadow-sm" style="width:45px; height:45px;">
                        ...
                        <td class="text-end pe-4 py-3">
                            <div class="d-flex justify-content-end gap-2">
                                <button class="btn btn-sm btn-white border shadow-sm hover-lift" data-bs-toggle="modal" data-bs-target="#editModal{{ $item->id }}" title="Edit Anggota">
                                    <i data-lucide="edit-3" class="icon-xs text-primary"></i>
                                </button>
                                <form action="{{ route('operator.bpd.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus anggota BPD ini?')">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="btn btn-sm btn-white border shadow-sm hover-lift" title="Hapus Anggota">
                    ...
                    <tr>
                        <td colspan="4" class="text-center py-5 text-muted bg-white">
                            ...
                            Belum ada anggota BPD yang diinput.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
...
<div class="modal fade" id="editModal{{ $item->id }}" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered text-start">
    <form action="{{ route('operator.bpd.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="modal-content border-0 shadow-lg rounded-4">
      @csrf @method('PUT')
      <div class="modal-header border-0 bg-success text-white py-3">
        <h5 class="modal-title fw-bold"><i data-lucide="edit-3" class="icon-sm me-2"></i> Edit Anggota BPD</h5>
...
<div class="modal fade" id="addModal" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered text-start">
    <form action="{{ route('operator.bpd.store') }}" method="POST" enctype="multipart/form-data" class="modal-content border-0 shadow-lg rounded-4">
      @csrf
      <div class="modal-header border-0 bg-success text-white py-3">
        <h5 class="modal-title fw-bold"><i data-lucide="plus" class="icon-sm me-2"></i> Tambah Anggota BPD</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body p-4">
        <div class="mb-3">
            <label class="small fw-bold text-muted mb-1">Nama Lengkap</label>
            <input type="text" name="nama" class="form-control rounded-3" placeholder="Nama tanpa/dengan gelar" required>
        </div>
        <div class="mb-3">
